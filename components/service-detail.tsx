...
export default function ServiceDetail({ serviceId }: ServiceDetailProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!service?.service_images || service.service_images.length <= 1) return
      
      if (e.key === 'ArrowLeft') {
        setSelectedImageIndex(prev => prev > 0 ? prev - 1 : service.service_images.length - 1)
      } else if (e.key === 'ArrowRight') {
        setSelectedImageIndex(prev => prev < service.service_images.length - 1 ? prev + 1 : 0)
      }
    }
  ...
  const fetchService = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Set seller profile if it's included in the service data
        if (data.service?.seller_profile) {
          setSellerProfile(data.service.seller_profile)
        }
      } else {
  ...
  const currentImage = images[selectedImageIndex] || service.primary_image
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="container mx-auto px-2 sm:px-4 py-2 sm:py-8">
          ...
          <div className="lg:col-span-2 space-y-6 lg:space-y-8">
             ...
             <Card className="overflow-hidden border-0 shadow-xl pt-0 bg-white/80 backdrop-blur-sm">
               <div className="relative">
                 {currentImage ? (
                   <Image
                     src={typeof currentImage === 'string' ? currentImage : currentImage.image_url}
                     alt={service.title}
                     width={600}
                     height={400}
                 ...
                 {images.length > 1 && (
                   <>
                     <Button 
                       ...
                       title="الصورة السابقة (السهم الأيسر)"
                     >
                       ←
                     </Button>
                     <Button 
                       ...
                       title="الصورة التالية (السهم الأيمن)"
                     >
                       →
                     </Button>
                   </>
                   ...
                   <div className="flex gap-2 overflow-x-auto">
                     {displayImages.map((image, index) => (
                       <Image
                         ...
                         width={100}
                         height={80}
                         className={`w-20 h-16 object-cover rounded-lg cursor-pointer hover:opacity-80 transition-all hover:scale-105 border-2 flex-shrink-0 ${
        ...
        {service && service.service_packages && service.service_packages[selectedPackage] && (
          <OrderConfirmationDialog
            isOpen={showOrderDialog}
            onClose={() => setShowOrderDialog(false)}
            serviceTitle={service.title}
            serviceId={service.id}
            selectedPackage={service.service_packages[selectedPackage]}
            sellerProfile={sellerProfile || { display_name: "البائع", is_verified: false }}
